    ...
    public function userLogin($email, $password)
    {
        ...
        $result = $this->db->select(
            "SELECT * FROM `utilisateur`
             WHERE `email` = :u AND `mot_de_passe` = :p",
            array("u" => $email, "p" => $password)
        );
        ...
        $this->updateLoginDate($result[0]['id_utilisateur']);
        ASSession::set("id_utilisateur", $result[0]['id_utilisateur']);
